				       ...
				       uint32_t event, void *data)
{
	...
	case TRXCON_EV_RX_DATA_IND:
		// Для проверки—выводим сообщение напрямую в stderr
		fprintf(stderr, "-----------------RX_DATA_IND event reached--------------------------------\n");
	  
		// Приводим указатель на структуру
		const struct trxcon_param_rx_data_ind *ind = (const struct trxcon_param_rx_data_ind *)data;
	   
		// Вывод основных значений с помощью логирования (здесь LOGL_ERROR гарантированно отображается)
		LOGPFSML(fi, LOGL_ERROR, "RX_DATA_IND: frame_nr=%u, chan_nr=%u, rssi=%d, n_errors=%u",
		   ind->frame_nr, ind->chan_nr, ind->rssi, ind->n_errors);
	   
		// Формируем строку с hex-выводом первых 16 байт данных
		int max_bytes = 16;
		char dump_buf[256] = {0};
		char *p = dump_buf;
		for (int i = 0; i < max_bytes; i++) {
		 p += sprintf(p, "%02x ", ind->data[i]);
		}
		LOGPFSML(fi, LOGL_ERROR, "Data (first %d bytes): %s", max_bytes, dump_buf);
	   
		// Продолжаем обработку данных, передавая их в GSM-стек
		l1ctl_tx_dt_ind(trxcon, ind);
		break;
	}
}
